...
async function callAIScreening(systemPrompt: string, userPrompt: string): Promise<AIEvaluationResult | null> {
  ...
  return {
    aiScore: Math.max(0, Math.min(100, Math.round(evaluationData.overallScore))),
...
export async function refineApplicationAsync(
  applicationId: string,
  additionalContext: string,
): Promise<AIEvaluationResult | null> {
  if (!openai) {
    console.warn('⚠️ Azure OpenAI not configured, skipping refinement');
    return null;
  }
  ...
  if (app.challengeId) {
    const [challenge] = await db.select().from(challenges).where(eq(challenges.id, app.challengeId));
    if (challenge?.evaluationCriteria) challengeEvalCriteria = challenge.evaluationCriteria;
  }
...
The admin has reviewed the previous AI evaluation and provided additional context that the AI may have missed. Take this into account and update your scoring accordingly.
...
Additional context from admin reviewer:
${additionalContext}`;

  const result = await callAIScreening(systemPrompt, userPrompt);
  if (!result) {
  ...
  console.log(`✅ Application ${applicationId} refined (score: ${result.aiScore})`);
  return result;
}
...
Validation Details: ${app.validationDetails || 'N/A'}`;
    ...
    console.log(`✅ Application ${applicationId} AI-reviewed (score: ${result.aiScore}) — awaiting Super Admin decision`);
  } catch (error) {
