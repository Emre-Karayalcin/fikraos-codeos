...
export const UsersTable: React.FC<UsersTableProps> = ({
  ...
  // local copy of rows to ensure UI updates when react-query returns new data
  const [localRows, setLocalRows] = useState<User[]>([]);
  React.useEffect(() => {
    setLocalRows(data?.rows || []);
    // debug logging to see param/data changes in browser console
    // eslint-disable-next-line no-console
    console.debug('UsersTable params:', params, 'rows length:', data?.rows?.length ?? 0, 'count:', data?.count);
  ...
  const estimated = (params.page - 1) * params.limit + rows.length + (rows.length === params.limit ? params.limit * UNKNOWN_PAGE_ALLOWANCE : 0);
